    ...
    try
    {
        ...
        while (stop == 0)
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                {
                    std::cout << "EPOLLIN event for fd " << fd << std::endl;
                    int recv_result = client.at(fd).receiveRequest();
                    std::cout << "receiveRequest() returned: " << recv_result << std::endl;
                    if (recv_result <= 0)
                    {
                        std::cout << "receiveRequest failed, disconnecting client" << std::endl;
                        disconnectClient(fd, client, epoll_fd);
                        continue;
                    }
                    else
                ...
                else if (events[i].events & EPOLLOUT)
                {
                    ...
                    std::cout << "\033[32mResponse sent, switching back to EPOLLIN\033[0m" << std::endl;
                    client.at(fd).set_response("");
                    client.at(fd).clearRequest(); // Clear request data for next request
                    struct epoll_event ev;
                    ev.events = EPOLLIN;
                    ev.data.fd = fd;
                    int e = epoll_ctl(epoll_fd, EPOLL_CTL_MOD, fd, &ev);
                    if (e == -1)
                    {
